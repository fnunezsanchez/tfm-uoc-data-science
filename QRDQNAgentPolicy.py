...
class QRDQNAgentPolicy(QRDQNPolicy):
    ...
    def forward(self, observation):
        ...
        # Return the action and the quantiles
        return q_values
    ...
    def save_image(self, observation):
        ...
        image = observation[:HEIGHT * WIDTH * 3].reshape((HEIGHT, WIDTH, 3))

        image = image.detach().cpu().numpy()
        image = image.astype(np.uint8)

        # Obtener el tiempo actual y convertirlo a un string
        now = datetime.now()
        timestamp = now.strftime("%Y%m%d_%H%M%S")

        # Usar el timestamp como parte del nombre del archivo
        filename = f'./logs/net_images/x_image_{timestamp}.jpg'
        Image.fromarray(image).save(filename, format='JPEG')
